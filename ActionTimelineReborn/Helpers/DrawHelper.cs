...
internal static class DrawHelper
{
    ...
    {
        // Texture loading must happen on the main thread, so resolve it here (during draw) instead of in the ctor.
        if (!ThreadLoadImageHandler.TryGetTextureWrap("ui/uld/icona_frame_hr1.tex", out IDalamudTextureWrap? _roundTex) || _roundTex == null)
        {
            return;
        }

        float pixPerUnit = size / 82;

        Vector2 outPos = position - new Vector2(pixPerUnit * 31, pixPerUnit * 31);
    ...
    public static void DrawActionIcon(this ImDrawListPtr drawList, uint iconId, bool isHq, Vector2 position, float size)
    {
        ...
        //Cover.
        if (ThreadLoadImageHandler.TryGetTextureWrap("ui/uld/icona_frame_hr1.tex", out IDalamudTextureWrap? frameText))
        {
            Vector2 coverPos = position - new Vector2(pixPerUnit * 3, pixPerUnit * 4);
